...
if(document.getElementById("tableLecturerTP")){
	let tableLecturerTP = new DataTable('#tableLecturerTP');

$('#tableLecturerTP').DataTable().destroy();

$(document).ready(function() {
    $('#tableLecturerTP').DataTable({
        language: {
            "sEmptyTable": "Không có dữ liệu",
            "sInfo": "Hiển thị từ _START_ đến _END_ (tổng: _TOTAL_) ",
        ...
        lengthMenu: [15, 25, 35],
        columnDefs: [
            { "orderable": false, "targets": [0,1,2,3,4,5] } // Thay [0, 1, 2] bằng các chỉ mục của các cột mà bạn muốn tắt sorting
        ],
