...
//	@Query("select d from Document d where d.module.id = :id AND d.module.name like %:name% AND d.type like %:type% OR ")
	@Query("SELECT d FROM Document d WHERE (:id IS NULL OR d.module.id = :id) AND (:name IS NULL OR d.module.name LIKE %:name%) AND (:type IS NULL OR d.type LIKE %:type%) OR (:name IS NULL or d.module.code like %:name%) AND (:type IS NULL OR d.type LIKE %:type%)")
